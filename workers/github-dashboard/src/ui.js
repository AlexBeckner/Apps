...
(function () {
  ...
  function renderSync() {
    ...
    syncRoot.innerHTML =
      '<div class="last-sync" title="' + attr(sourcesSummary(sync)) + '"><div>Last synced</div><strong>' + shortRelative(sync.lastSyncAt) + '</strong></div>' +
  ...
  function labelForPhase(phase) {
    var labels = { starting: "Starting...", dispatch: "Dispatching...", repo: "Repo...", branches: "Branches...", tags: "Tags...", commits: "Commits...", prs: "PRs...", done: "Done..." };
    return labels[phase] || "Syncing...";
  }

  // Per-source last-synced times (branches / commits / PRs / tags) for the
  // "Last synced" hover tooltip, so each git-based sync is verifiable at a glance.
  function sourcesSummary(sync) {
